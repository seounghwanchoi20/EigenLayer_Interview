...
class GameServer {
    ...
    handleBattleAction(playerId, data) {
        console.log(`[Battle Action] Starting battle action for player ${playerId}`);
    ...
    handleBattleReady(playerId, battleId) {
        ...
        if (battle.readyPlayers.has(battle.player1) &&
            battle.readyPlayers.has(battle.player2)) {
            console.log(`[Battle ${battleId}] Both players are ready, starting battle!`);
            const player1 = this.players.get(battle.player1);
            const player2 = this.players.get(battle.player2);
            if (!player1 || !player2) {
                console.error(`[Battle ${battleId}] Cannot start battle - missing players:`, {
            ...
            console.log(`[Battle ${battleId}] Sending battle_start messages with health:`, {
                player1Message,
                player2Message,
                currentHealth: {
            ...
            const opponent = this.players.get(opponentId);
            if (opponent) {
                console.log(`[Battle ${battleId}] Notifying opponent ${opponentId} that player ${playerId} is ready`);
                opponent.ws.send(JSON.stringify({
                    type: "opponent_ready",
                    battleId: battleId,
                }));
                ...
                console.warn(`[Battle ${battleId}] Cannot notify opponent ${opponentId} - not found`);
            }
        }
    }
