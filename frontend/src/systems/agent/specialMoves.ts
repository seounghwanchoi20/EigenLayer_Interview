...
export function calculateMoveDamage(
  ...
  switch (move.type) {
    case "Physical":
      baseDamage *=
        (attackerStats.strength / 300) * (1 - defenderStats.defense! / 600);
      break;
    case "Magical":
      baseDamage *=
        (attackerStats.magic! / 300) * (1 - defenderStats.intelligence / 600);
      break;
    case "Ultimate":
      baseDamage *= (attackerStats.strength + attackerStats.magic!) / 400;
      break;
  }

  // Add larger random variation (±10%)
